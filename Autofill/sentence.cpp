...
void Sentence::complete(const Dict &d){
	...
	vector<string> temp = d.sentences;
	vector<int> clen;
	...
	for(x=temp.begin(); x!=temp.end(); x++){
		...
		if (st.compare(0,input_sentence.length(),input_sentence) == 0){ // position 0 to end of input_sentence, compared to any part of sentences[i] of the input sentence length
...
void Sentence::check(const Dict &d){
	vector<int> hd;
	int cdist;
	int in_len=input_sentence.length();
	int l;
	string in_str;
...
void Sentence::show(){
	endl (cout);
	cout<<"SENTENCE completion for: "<<input_sentence<<endl;
	cout<<"------------------------------"<<endl;
	...
	cout<<"SENTENCE correction for: "<<input_sentence<<endl;
	cout<<"------------------------------"<<endl;
