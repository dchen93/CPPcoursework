#include <iostream>
#include <climits>
#include "LeafNode.h"
#include "InternalNode.h"
#include "QueueAr.h"

using namespace std;


LeafNode::LeafNode(int LSize, InternalNode *p,
  BTreeNode *left, BTreeNode *right) : BTreeNode(LSize, p, left, right)
{
  values = new int[LSize];
}  // LeafNode()

void LeafNode::addToLeft(int value, int last)
{
  leftSibling->insert(values[0]);

  for(int i = 0; i < count - 1; i++)
    values[i] = values[i + 1];

  values[count - 1] = last;
  if(parent)
    parent->resetMinimum(this);
} // LeafNode::ToLeft()

void LeafNode::addToRight(int value, int last)
{
  rightSibling->insert(last);

  if(value == values[0] && parent)
    parent->resetMinimum(this);
} // LeafNode::addToRight()

void LeafNode::addToThis(int value)
{
  int i;

  for(i = count - 1; i >= 0 && values[i] > value; i--)
      values[i + 1] = values[i];

  values[i + 1] = value;
  count++;

  if(value == values[0] && parent)
    parent->resetMinimum(this);
} // LeafNode::addToThis()


void LeafNode::addValue(int value, int &last)
{
  int i;

  if(value > values[count - 1])
    last = value;
  else
  {
    last = values[count - 1];

    for(i = count - 2; i >= 0 && values[i] > value; i--)
      values[i + 1] = values[i];
    // i may end up at -1
    values[i + 1] = value;
  }
} // LeafNode:: addValue()


int LeafNode::getMaximum()const
{
  if(count > 0)  // should always be the case
    return values[count - 1];
  else
    return INT_MAX;
} // getMaximum()


int LeafNode::getMinimum()const
{
  if(count > 0)  // should always be the case
    return values[0];
  else
    return 0;

} // LeafNode::getMinimum()


LeafNode* LeafNode::insert(int value)
{
  int last;

  if(count < leafSize)
  {
    addToThis(value);
    return NULL;
  } // if room for value

  addValue(value, last);

  if(leftSibling && leftSibling->getCount() < leafSize)
  {
    addToLeft(value, last);
    return NULL;
  }

  else // left sibling full or non-existent
    if(rightSibling && rightSibling->getCount() < leafSize)
    {
      addToRight(value, last);
      return NULL;
    }
    else // both siblings full or non-existent
      return split(value, last);
}  // LeafNode::insert()

void LeafNode::print(Queue <BTreeNode*> &queue)
{
  cout << "Leaf: ";
  for (int i = 0; i < count; i++)
    cout << values[i] << ' ';
  cout << endl;
} // LeafNode::print()

void LeafNode::removeValue(int value, int &last){
  int i, k;

  for (i = 0; i < count; i++){
    if (values[i] == value){
      for (k = i; k < count - 1; k++)
        values[k] = values[k+1];
      count--;
      last = values[count - 1];
    }
  }
}


void LeafNode::borrowFromLeft()
{
  insert(leftSibling->getMaximum());
  leftSibling->remove(leftSibling->getMaximum());
  parent->resetMinimum(this);
}

void LeafNode::borrowFromRight()
{
  insert(rightSibling->getMinimum());
  rightSibling->remove(rightSibling->getMinimum());
  parent->resetMinimum(this);
}

LeafNode* LeafNode::remove(int value)
{   // To be written by students
  int last;
  
  removeValue(value, last);

  //for leafsize of 2
  if(leftSibling && leafSize == 2 && (leftSibling->getCount() > 1) && (this->getCount() < 1))
  {
    borrowFromLeft();
    return NULL;
  }
  else // left sibling non-existent, or current leaf is less than half capacity
    if(rightSibling && leafSize == 2 && (rightSibling->getCount() > 1) && (this->getCount() < 1))
    {
      borrowFromRight();
      return NULL;
    }
  // done taking care of leafsize of 2

  if(count == 0)
    return NULL;
  
  if(leftSibling && (leftSibling->getCount() > (leafSize / 2 + leafSize % 2)) && (this->getCount() < leafSize / 2 + leafSize % 2))
  {
    borrowFromLeft();
    return NULL;
  }
  else if(leftSibling && (leftSibling->getCount() + count) <= leafSize && (count < leafSize/2 + leafSize % 2))
  {
    return merge(1);
  }
  else // left sibling non-existent, or current leaf is less than half capacity
    if(rightSibling && (rightSibling->getCount() > (leafSize / 2 + leafSize % 2)) && (this->getCount() < leafSize / 2 + leafSize % 2))
    {
      borrowFromRight();
      return NULL;
    }
    else if(rightSibling && (rightSibling->getCount() + count) <= leafSize && (count < leafSize/2 + leafSize % 2))
    {
      return merge(0);
    }

  return NULL;  // filler for stub
}  // LeafNode::remove()


LeafNode* LeafNode::split(int value, int last)
{
  LeafNode *ptr = new LeafNode(leafSize, parent, this, rightSibling);


  if(rightSibling)
    rightSibling->setLeftSibling(ptr);

  rightSibling = ptr;

  for(int i = (leafSize + 1) / 2; i < leafSize; i++)
    ptr->values[ptr->count++] = values[i];

  ptr->values[ptr->count++] = last;
  count = (leafSize + 1) / 2;

  if(value == values[0] && parent)
    parent->resetMinimum(this);
  return ptr;
} // LeafNode::split()

LeafNode* LeafNode::merge(int mergewithleft)
{
  LeafNode *ptr = new LeafNode(leafSize, parent, this, rightSibling);

  if(leftSibling && (leftSibling->getCount() + count) <= leafSize && mergewithleft == 1){
    leftSibling->setRightSibling(rightSibling);
    //rightSibling->setLeftSibling(leftSibling);

    for(int i = 0; i < count; i++){
      leftSibling->insert(values[i]);
      remove(values[i]);
    }
    ptr = this; // should be leftSibling
  }
  else if(rightSibling && (rightSibling->getCount() + count) <= leafSize && mergewithleft == 0){
    rightSibling->setLeftSibling(leftSibling);
    //leftSibling->setRightSibling(rightSibling);

    for(int i = 0; i < count; i++){
      rightSibling->insert(values[i]);
      remove(values[i]);
    }
    ptr = this; // should be rightSibling
  }
  return ptr;
}
